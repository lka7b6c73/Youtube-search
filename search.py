...
url = "https://dc5e-117-6-40-134.ngrok-free.app"
client = MongoClient("mongodb://localhost:27017/")  # Thay đổi URL này nếu bạn kết nối từ xa
db = client["YoutubeAPI"]  # Kết nối đến database YoutubeAPI
collection = db["users"]
...
def search_videos(query):
    ...
    return videos, time_load, time_search
...
def click_button():   
    ...
    if search_data[0]:
        ...
        st.session_state.search_results, st.session_state.time_load, st.session_state.time_search = search_data
        st.session_state.info = (st.session_state.query,st.session_state.search_results, st.session_state.time_load, st.session_state.time_search)
...
# Giao diện người dùng Streamlit
    ...
    if 'history' in st.session_state and st.session_state.history:
        for i, (iquery, iresult, iload, itime) in enumerate(st.session_state.history):
            if st.sidebar.button(f"🔗{iquery}", key=f"history_{i}"):
                st.session_state.info = (iquery,iresult,iload,itime)
                st.session_state.search_triggered = True
                st.session_state.account = False
    ...
    # Sắp xếp dữ liệu theo ngày xuất bản từ mới nhất đến cũ nhất
        ...
        if info[1]:
            ...
            if info[1]:
                ...
                with col1:
                    st.write(f'Thời điểm: {info[3]} ')
                with col2:
                    st.write(f" Thời gian tìm kiếm '{info[0]}': {info[2]}s")   
                ### bảng ngày 
                col1, col2 = st.columns([1,1]) 
                with col1:
                ...
                results_filtered = [item for item in info[1] if start_date_str <= item["Publish Time"] <= end_date_str]
                ...
                start_index, end_index = st.slider(
                    f"Tìm thấy {len(results_filtered)} kết quả ✅",
                    min_value=1,
                    max_value=len(results_filtered),  # Cần điều chỉnh tối đa theo số lượng video có thể có
                ...
                videos_to_display = results_sorted[start_index-1:end_index]
